...
/**
 * Intel 8059A Programmable Interrupt Controller.
 * <p>
 * The 8059A accepts two types of command words generated by the CPU:
 * <ul>
 * <li>Initialization Command Words (ICWs): Before normal operation can begin,
 * each 8259A in the system must be brought to a starting point - by a sequence
 * of 2 to 4 bytes.</li>
 * <li>Operation Command Words (OCWs): These are the command words which command
 * the 8259A to operate in various interrupt modes. These modes are:
 * <ol>
 * <li>Fully nested mode</li>
 * <li>Rotating priority mode</li>
 * <li>Special mask mode</li>
 * <li>Polled mode</li>
 * </ol>
 * </li>
 * </ul>
...
public class I8259 implements InterruptController, IOPortHandler {
    private static final Logger logger = Logger.getLogger(I8259.class.getName());
	...
	public IrqLine(int i) {
	    irq = i;
	}

	@Override
	public void raise(boolean enable) {
	    raiseIrq(irq, enable);
	}
    }
    ...
    /**
     * 
     * @return the vector number of a pending interrupt request, or {@code -1} if no
	...
	// TODO log.debug("pendinginterrupt irq=" + irq + " vector=" + ((baseVector +
	// irq) & 0xff));
	...
	// TODO log.debug("raiseIrq " + irq + " " + enable);
	if (enable) {
	    rIRR |= (1 << irq);
	} else {
	    rIRR &= ~(1 << irq);
    ...
    /**
     * Handle write request from the {@link Cpu}.
     * <p>
     * On the IBM PC the address decoder for PIC places two registers in the I/O
     * address space at locations 20h and 21h.
     */
    @Override
    ...
    /**
     * Handle read request from the {@link Cpu}.
     * <p>
     * On the IBM PC the address decoder for PIC places two registers in the I/O
     * address space at locations 20h and 21h.
     */
    ...
    /**
     * Handle Initialization Control Word 1 (ICW1).
     * <p>
     * ICW1 starts the initialization sequence during which the following
     * automatically occur.
     * <ol>
     * <li>The edge sense circuit is reset</li>
     * <li>The Interrupt Mask Register is cleared.</li>
     * <li>IR7 input is assigned priority 7.</i>
     * <li>The slave mode addrss is set to 7.</li>
     * <li>Special Mask Mode is cleared and Status Read is set to IRR</li>
     * <li>If IC4 = 0, then all functions selected in ICW4 are set to zero.</li>
     * </ol>
     * 
     * @param v
     */
	...
	if (logger.isLoggable(Level.FINE)) {
	    logger.log(Level.FINE, "levelTriggered={0}", levelTriggered);
	}
    }
	...
	baseVector = v & 0xf8;
	state = (expectICW3) ? ((expectICW4) ? stICW4 : stREADY) : stICW3;

	if (logger.isLoggable(Level.FINE)) {
	    logger.log(Level.FINE, "baseVector={0}", Misc.byteToHex(baseVector));
	}
    }
	...
	if (logger.isLoggable(Level.FINE)) {
	    logger.log(Level.FINE, "autoEOI={0}, specialNest={1}", new Object[] { autoEOI, specialNest });
	}
    }
	...
	if (logger.isLoggable(Level.FINE)) {
	    logger.log(Level.FINE, "OCW1: IMR={0}", Misc.byteToHex(rIMR));
	}
    }
