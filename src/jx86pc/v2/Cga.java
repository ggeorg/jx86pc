 ...
 * 
 * @see https://www.seasip.info/VintagePC/cga.html
 * @see http://minuszerodegrees.net/oa/OA%20-%20IBM%20Color%20Graphics%20Monitor%20Adapter%20(CGA).pdf
 */
public class Cga implements IOPortHandler
{

    public static final int BASEADDR = 0xb8000; 
    	// TODO: the frame buffer is repeated at 0xbc000
    public static final int MEMSIZE =  0x04000; 
    	// 16 kByte of RAM
    public static final int BASEPORT = 0x03d0; 
    	// I/O address used are 0x03d0-0x03df
    ...
    public void reset()
    {
        ...
        crtReg[0]  = 0x71; // Horizontal Total
        crtReg[1]  = 0x50; // Horizontal Displayed
        crtReg[2]  = 0x5A; // Horizontal Sync Position
        crtReg[3]  = 0x0A; // Horizontal Sync Width
        crtReg[4]  = 0x1F; // Vertical Total
        crtReg[5]  = 0x06; // Vertical Total Adjust
        crtReg[6]  = 0x19; // Vertical Displayed
        crtReg[7]  = 0x1C; // Vertical Sync Position
        crtReg[8]  = 0x02; // Interlace Mode
        crtReg[9]  = 0x07; // Maximum Scan Line Address
        crtReg[10] = 0x06; // Cursor Start
        crtReg[11] = 0x07; // Cursor End
        
        crtReg[12] = 0x00; // Start Address (H)
        crtReg[13] = 0x00; // Start Address (L)
        crtReg[14] = 0x00; // Cursor Address (H)
        crtReg[15] = 0x00; // Cursor Address (L)
        crtReg[16] = 0x00; // Light Pen (H)
        crtReg[17] = 0x00; // Light Pen (L)
        
        for (int i = 18; i < 32; i++)
            crtReg[i] = 0;

        videoMode = 1; // 80x25 text mode
        enabledVideo = true;
        fullUpdate = true;
    ...
    {
        int newmode;
        if ((modeReg & 0x02) == 0) // text mode?
            newmode = ((modeReg & 0x01) == 0) ? 0 : 1; // 0:40x25 1:80x25
        else
            newmode = ((modeReg & 0x10) == 0) ? 2 : 3; // 0:320x200 1:640x200
    ...
    private final int statusReg()
    {
        ...
        boolean hretrace = (t % ht) <= (ht / 10);
        	// According to IBM's manual, this bit is 1 when the 
        	// CPU is allowed to access RAM without causing "snow".
        boolean vretrace = (t % vt) <= (vt / 10);
        	// Set when the screen is in a vertical retrace interval. 
        	// While this bit is set, video RAM can be accessed without
        	// "snow" onscreen.
    ...
    private final void updatePalette()
    {
        ...
        if (videoMode == 2) {
            ...
            int intense = (colorReg & 0x10) >> 1; // 0x00:no, 0x08:yes
